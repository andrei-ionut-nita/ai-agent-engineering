# Phase B: Publish `corrective_rag` to the portfolio
...
- [ ] Run `/write-course` (portfolio) end to end for `corrective_rag` →
      `corrective-rag`, through its own approval gates
- [ ] Verify the published course: `npm run check:types` /
      `check:tokens` / `lint:css` / `build`, source-link integrity check,
      `npm run preview` visit to `/learning/corrective-rag/` and one
      lesson
...
2. Invoke the existing `/write-course` skill (`portfolio:write-course`)
   with:
   - `sourceSlug`: `corrective_rag`
   - `slug`: `corrective-rag`
   - `topic`: `Retrieval` (existing catalog value, no catalog edit needed)
   - `title`: "Corrective RAG: Grading and Fixing Retrieval Before You
     Generate" (from the Course 4 Spec in `phase-a-authoring.md`)
...
6. Once verified, update the Series Roadmap table in
   [`../README.md`](../README.md): `corrective_rag` status →
   "Published".
7. Report back to the user with the live/staged URL
   (`/learning/corrective-rag/`) and confirm whether they want it
   committed/pushed.
...
and a manual `npm run preview` visit to `/learning/corrective-rag/` and
one lesson page.
