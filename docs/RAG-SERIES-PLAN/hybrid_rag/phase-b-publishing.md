# Phase B: Publish `hybrid_rag` to the portfolio
...
- [ ] Run `/write-course` (portfolio) end to end for `hybrid_rag` →
      `hybrid-rag`, through its own approval gates
- [ ] Verify the published course: `npm run check:types` /
      `check:tokens` / `lint:css` / `build`, source-link integrity check,
      `npm run preview` visit to `/learning/hybrid-rag/` and one lesson
...
2. Invoke the existing `/write-course` skill (`portfolio:write-course`)
   with:
   - `sourceSlug`: `hybrid_rag`
   - `slug`: `hybrid-rag`
   - `topic`: `Retrieval` (existing catalog value, no catalog edit needed)
   - `title`: "Hybrid RAG: Combining Dense and Sparse Retrieval" (from
     the Course 2 Spec in `phase-a-authoring.md`)
...
6. Once verified, update the Series Roadmap table in
   [`../README.md`](../README.md): `hybrid_rag` status → "Published".
7. Report back to the user with the live/staged URL
   (`/learning/hybrid-rag/`) and confirm whether they want it
   committed/pushed.
...
and a manual `npm run preview` visit to `/learning/hybrid-rag/` and one
lesson page.
