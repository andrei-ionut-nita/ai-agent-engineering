# Phase B: Publish `agentic_rag` to the portfolio
...
- [ ] Run `/write-course` (portfolio) end to end for `agentic_rag` →
      `agentic-rag`, through its own approval gates
- [ ] Verify the published course: `npm run check:types` /
      `check:tokens` / `lint:css` / `build`, source-link integrity check,
      `npm run preview` visit to `/learning/agentic-rag/` and one lesson
...
2. Invoke the existing `/write-course` skill (`portfolio:write-course`)
   ...
   - `title`: "Agentic RAG: Retrieval as a Tool the Model Chooses to Use"
     (from the Course 5 Spec in `phase-a-authoring.md`)
...
6. Once verified, update the Series Roadmap table in
   [`../README.md`](../README.md): `agentic_rag` status → "Published".
7. Report back to the user with the live/staged URL
   (`/learning/agentic-rag/`) and confirm whether they want it
   committed/pushed.
...
and a manual `npm run preview` visit to `/learning/agentic-rag/` and one
lesson page.
