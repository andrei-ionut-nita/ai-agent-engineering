"""
Lesson 8: one script, one loop, three kinds of questions, retrieval,
the time tool, or neither, each handled correctly by the same ask().
...
def ask(query: str, store: list[dict]) -> tuple[str, str]:
    # Returns (answer, tool_used) so main() can print what actually
    # happened, "none", "search_notes", or "get_current_datetime".
...
def main() -> None:
    ...
    questions = [
        ("How often does Clarence the sourdough starter need feeding at room temperature?", "retrieval"),
        ("What time is it right now in Lisbon?", "the time tool"),
        ("What is 7 times 8?", "neither, straight arithmetic"),
    ]

    for question, expected_kind in questions:
        answer, tool_used = ask(question, store)
        print(f"Q ({expected_kind}): {question}")
        print(f"  tool used: {tool_used}")
        print(f"  A: {answer}\n")

    print(
        "One ask() function, one declared toolset, three different question\n"
        "shapes, each routed correctly without a single hardcoded if-statement\n"
        "about the question itself. That routing is the whole thesis of this\n"
        "course's Beginner tier: the model, not the pipeline, decides."
    )
