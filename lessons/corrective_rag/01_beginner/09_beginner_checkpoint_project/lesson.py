...
def corrective_ask(query: str, store: list[dict], k: int = 3) -> tuple[str, bool]:
    """Returns (answer, corrected) - corrected is True whenever naive
    top-1 retrieval alone would have used a different (or empty) context
    than what actually reached generation here."""
    ...
    return generate_answer(effective_query, relevant), corrected
...
def main() -> None:
    ...
    questions = [
        # Naive top-1 misses (weather-station.md), correction fixes it.
        ...
        # Naive top-1 already correct, nothing to correct.
        "What's the cold ferment time for the pizza dough?",
        # Out of corpus, correction fires but honestly can't fix it.
        "What is the capital of France?",
    ]

    for question in questions:
        answer, corrected = corrective_ask(question, store)
