"""
Lesson 8: one clean pipeline, classify + route + all three strategies +
generation, wired into a single answer() function.
...
def answer(query: str, store: list[dict]) -> dict:
    # The whole tier, in one function: classify the question, route it
    # to whichever of the three strategies matches its label, and
    # return both the answer and which route produced it. Every lesson
    # before this one built one piece; this is the piece that was being
    # built toward the whole time.
