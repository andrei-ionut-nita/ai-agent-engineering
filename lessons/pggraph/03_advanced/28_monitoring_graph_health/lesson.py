"""
Lesson 28: graph.status(), graph.resource_status(), graph.projection_status().
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        node_count, edge_count, sync_status, read_only = conn.execute("""
            SELECT node_count, edge_count, sync_status, read_only FROM graph.status()
        """).fetchone()
        print(
            f"status(): node_count={node_count}, edge_count={edge_count}, "
            f"sync_status={sync_status}, read_only={read_only}"
        )
        ...
        """).fetchall()
        print(f"resource_status(): {resource}")

        manifest_generation, segment_count, compaction_recommended = conn.execute("""
            SELECT manifest_generation, segment_count, compaction_recommended
            FROM graph.projection_status()
        """).fetchone()
        print(
            f"projection_status(): manifest_generation={manifest_generation}, "
            f"segment_count={segment_count}, compaction_recommended={compaction_recommended}"
        )
