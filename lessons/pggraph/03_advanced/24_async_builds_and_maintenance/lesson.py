"""
Lesson 24: async builds, vacuum, and maintenance.
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        build_id, status = conn.execute(
            "SELECT build_id, status FROM graph.build_async_graph('default')"
        ).fetchone()
        print(f"Async build: build_id={build_id} status={status}")

        # Poll until the background worker finishes. This course's tiny
        # example graph finishes almost instantly, a real, large graph
        # is exactly when this loop earns its keep.
        for _ in range(20):
            status, nodes_loaded, edges_loaded = conn.execute(
                "SELECT status, nodes_loaded, edges_loaded FROM graph.build_status(%s)",
                (build_id,),
            ).fetchone()
            if status == "completed":
                break
            time.sleep(0.2)
        print(f"After polling: status={status}, nodes_loaded={nodes_loaded}, edges_loaded={edges_loaded}")

        nodes_before, nodes_after, tombstones_removed, _edges_rebuilt, _ms = conn.execute(
            "SELECT * FROM graph.vacuum()"
        ).fetchone()
        print(
            f"Vacuum: nodes_before={nodes_before}, nodes_after={nodes_after}, "
            f"tombstones_removed={tombstones_removed}"
        )

        _job_id, status, sync_rows_applied, nodes_after, edges_after, _ms, _err = conn.execute(
            "SELECT * FROM graph.maintenance()"
        ).fetchone()
        print(
            f"Maintenance: status={status}, sync_rows_applied={sync_rows_applied}, "
            f"nodes_after={nodes_after}, edges_after={edges_after}"
        )
