"""
Lesson 27: graph.grant_graph(), graph.set_graph_quota(), graph.graph_quota_usage().
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        graph_name, grantee, privilege = conn.execute("""
            SELECT graph_name, grantee::regrole::text, privilege
            FROM graph.grant_graph('default', grantee := 'postgres', privilege := 'read')
        """).fetchone()
        print(f"Granted: {graph_name} -> {grantee} ({privilege})")

        scope_type, dimension, limit_value, enforcement = conn.execute("""
        ...
        """).fetchone()
        print(f"Quota set: {scope_type} / {dimension}, limit={limit_value}, enforcement={enforcement}")

        usage = conn.execute("""
            SELECT scope_type, limit_value, usage_value, exceeded
            FROM graph.graph_quota_usage()
            WHERE dimension = 'max_named_graphs' AND scope_type = 'cluster'
        """).fetchall()
        print(f"Quota usage (max_named_graphs): {usage}")
