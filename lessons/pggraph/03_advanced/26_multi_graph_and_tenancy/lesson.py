"""
Lesson 26: multiple named graphs, and the one-loaded-graph-per-backend rule.
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        print(f"default status: node_count={node_count}, edge_count={edge_count}")

        # create_graph() errors if 'archive' already exists, and there's
        # no function to unregister a table from a graph, so on a
        # rerun of this script, reuse the existing 'archive' graph
        # (registration + build are themselves safe to repeat) instead
        # of trying to drop and recreate it.
        exists = conn.execute(
            "SELECT 1 FROM graph.list_graphs() WHERE graph_name = 'archive'"
        ).fetchone()
        if not exists:
            conn.execute("SELECT graph.create_graph('archive')")
        graphs = conn.execute(
            "SELECT graph_name FROM graph.list_graphs() ORDER BY graph_name"
        ).fetchall()
        print(f"Graphs after create_graph('archive'): {[g for (g,) in graphs]}")

        conn.execute("SELECT graph.set_current_graph('archive')")
        conn.execute("""
            SELECT graph.add_table('public.companies'::regclass,
                id_column := 'id', columns := ARRAY['name'])
        """)
        ...
        print(f"archive status (after its own build): node_count={node_count}, edge_count={edge_count}")

        # Switching back doesn't restore default's compiled state, only
        # one graph is loaded per backend at a time.
        conn.execute("SELECT graph.set_current_graph('default')")
        node_count, edge_count = conn.execute(
            "SELECT node_count, edge_count FROM graph.status()"
        ).fetchone()
        print(f"default status, switched back, before rebuilding: node_count={node_count}, edge_count={edge_count}")

        conn.execute("SELECT graph.build()")
        node_count, edge_count = conn.execute(
            "SELECT node_count, edge_count FROM graph.status()"
        ).fetchone()
        print(f"default status, after rebuilding: node_count={node_count}, edge_count={edge_count}")
