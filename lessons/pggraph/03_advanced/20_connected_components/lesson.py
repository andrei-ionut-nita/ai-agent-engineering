"""
Lesson 20: graph.component_stats(), graph.components(), graph.isolated_nodes().
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        num_components, largest_component, num_isolated, total_active = conn.execute(
            "SELECT num_components, largest_component, num_isolated_nodes, "
            "total_active_nodes FROM graph.component_stats()"
        ).fetchone()
        print(
            f"component_stats(): num_components={num_components}, "
            f"largest_component={largest_component}, "
            f"num_isolated_nodes={num_isolated}, total_active_nodes={total_active}"
        )

        sizes = conn.execute(
            "SELECT component_size FROM graph.components(max_rows := 20)"
        ).fetchall()
        print(f"components(): {[s for (s,) in sizes]}")

        isolated = conn.execute(
            "SELECT node_id, node ->> 'name' FROM graph.isolated_nodes(max_rows := 100)"
        ).fetchall()
        print(f"isolated_nodes(): {isolated}")
