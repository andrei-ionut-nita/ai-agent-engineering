"""
Lesson 18: sync overlays and graph.apply_sync().
...
def reports_to_alice(conn: psycopg.Connection) -> list[str]:
    rows = conn.execute("""
        SELECT node_id FROM graph.traverse(
            seed_table := 'public.people'::regclass, seed_id := 'p1',
            max_depth := 2, edge_types := ARRAY['reports_to'],
            direction := 'in', hydrate := false
        )
    """).fetchall()
    return [node_id for (node_id,) in rows]


def pending_sync_rows(conn: psycopg.Connection) -> int:
    return conn.execute("SELECT pending_sync_rows FROM graph.status()").fetchone()[0]
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        print(f"Reports to Alice, before Carol's manager change: {reports_to_alice(conn)}")

        conn.execute("UPDATE people SET manager_id = 'p1' WHERE id = 'p3'")
        print(f"pending_sync_rows after the UPDATE (no sync yet): {pending_sync_rows(conn)}")
        print(f"Reports to Alice, immediately after the UPDATE: {reports_to_alice(conn)}")

        inserts, updates, deletes = conn.execute("SELECT * FROM graph.apply_sync()").fetchone()
        print(f"apply_sync: inserts={inserts}, updates={updates}, deletes={deletes}")
        print(f"pending_sync_rows after apply_sync: {pending_sync_rows(conn)}")
