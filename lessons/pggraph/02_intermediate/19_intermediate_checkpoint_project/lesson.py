"""
Lesson 19 (Intermediate Checkpoint): Reporting Chain Finder.

Combines edge_types-restricted traversal (Lesson 10) and filtered
traversal (Lesson 11) into a small reporting-structure report.
...
def management_chain(conn: psycopg.Connection, person_id: str) -> list[tuple[int, str, str]]:
    """Walks reports_to outward: subordinate -> manager -> manager's manager."""
    rows = conn.execute(
        """
        SELECT depth, node_id, node ->> 'name' AS name
        FROM graph.traverse(
            seed_table := 'public.people'::regclass, seed_id := %s,
            max_depth := 10, edge_types := ARRAY['reports_to'],
            direction := 'out', include_start := false, hydrate := true
        )
        ORDER BY depth
        """,
        (person_id,),
    ).fetchall()
    return [(depth, node_id, name) for depth, node_id, name in rows]


def senior_reports(
    conn: psycopg.Connection, person_id: str, min_years: int
) -> list[tuple[int, str, str, int]]:
    """Walks reports_to inward: manager -> direct/indirect reports,
    filtered to those with at least min_years of seniority."""
    rows = conn.execute(
        """
        SELECT depth, node_id, node ->> 'name' AS name,
               (node ->> 'seniority_years')::int AS seniority_years
        FROM graph.traverse(
            seed_table := 'public.people'::regclass, seed_id := %s,
            max_depth := 10, edge_types := ARRAY['reports_to'],
            direction := 'in', include_start := false, hydrate := true,
            filter := graph.gte('seniority_years', %s)
        )
        ORDER BY depth
        """,
        (person_id, min_years),
    ).fetchall()
    return [(depth, node_id, name, years) for depth, node_id, name, years in rows]


def print_report(conn: psycopg.Connection, person_id: str, person_name: str) -> None:
    print(f"Management chain for {person_name} (up to the top):")
    chain = management_chain(conn, person_id)
    if not chain:
        print("  (none, they're already at the top)")
    for depth, node_id, name in chain:
        print(f"  depth={depth}  {name} ({node_id})")

    print(f"\n{person_name}'s reports with 3+ years of seniority:")
    reports = senior_reports(conn, person_id, 3)
    if not reports:
        print(f"  (none, {person_name} doesn't manage anyone senior enough)")
    for depth, node_id, name, years in reports:
        print(f"  depth={depth}  {name} ({node_id}), {years} years")
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        print_report(conn, "p4", "Dan")
        print()
        print_report(conn, "p1", "Alice")
