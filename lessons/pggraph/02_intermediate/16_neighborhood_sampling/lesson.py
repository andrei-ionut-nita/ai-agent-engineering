"""
Lesson 16: graph.neighborhood(), sampling a neighborhood's shape.
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        rows = conn.execute("""
            SELECT depth, node_table_name, node_count, sample_nodes
    ...
    print("Neighborhood of Alice (depth<=2, sample_k=5):")
    for depth, node_table_name, node_count, sample_nodes in rows:
        print(
            f"  depth={depth}  {node_table_name:<10} "
            f"node_count={node_count}  sample={sample_nodes}"
        )
