"""
Lesson 17: graph.preview_discover(), a safe dry run of auto-discovery.
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        # Read-only: reports what graph.auto_discover() would register,
        # without registering anything. Deliberately not calling the
        # mutating auto_discover() here, see the README for why.
        rows = conn.execute("""
            SELECT item_type, item_name, details
            FROM graph.preview_discover('public')
        """).fetchall()

    print("preview_discover('public') would register:")
    for item_type, item_name, details in rows:
        print(f"  {item_type:<6} {item_name:<44} {details}")
