"""
Lesson 11: filtering a traversal by node property.
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        # graph.gte() builds the JSONB filter graph.traverse()'s `filter`
        # parameter expects; seniority_years was registered for
        # filtering via graph.add_filter_column() in setup_and_build().
        rows = conn.execute("""
            SELECT depth, node_table_name, node_id
            FROM graph.traverse(
                seed_table := 'public.people'::regclass, seed_id := 'p1',
                max_depth := 2, edge_types := ARRAY['reports_to'],
                direction := 'in',
                filter := graph.gte('seniority_years', 5),
                hydrate := false
            )
        """).fetchall()

    print("Reports to Alice (any depth<=2) with seniority_years >= 5:")
    for depth, node_table_name, node_id in rows:
        print(f"  depth={depth}  {node_table_name}  {node_id}")
