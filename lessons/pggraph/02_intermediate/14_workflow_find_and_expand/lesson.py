"""
Lesson 14: graph.find() and graph.expand(), the higher-level workflow pair.
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        node_table_name, node_id, node = conn.execute("""
        ...
        """).fetchone()
        print(f"find('Alice'): {node_table_name} {node_id} {node}")

        # direction is explicit here, expand()'s default 'any' doesn't
        # reliably walk a one-way edge like reports_to in reverse, see
        # the README.
        rows = conn.execute("""
            SELECT depth, node_table_name, node_id FROM graph.expand(
    ...
    print("expand(reports_to, in, seniority_years > 3):")
