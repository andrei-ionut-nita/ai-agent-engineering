"""
Lesson 13: graph.weighted_shortest_path(), summing a weight column
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        # in setup_and_build(), that's what makes this call possible.
        rows = conn.execute("""
            SELECT step, node_table_name, node_id, edge_weight, step_cost, total_cost
    ...
    print("Weighted shortest path, Acme Wealth -> Acme Bank (by ownership_pct):")
    for step, node_table_name, node_id, edge_weight, step_cost, total_cost in rows:
        suffix = f"  edge_weight={edge_weight}" if edge_weight is not None else ""
        print(
            f"  step={step}  {node_table_name}  {node_id}"
            f"  step_cost={step_cost}  total_cost={total_cost}{suffix}"
        )
