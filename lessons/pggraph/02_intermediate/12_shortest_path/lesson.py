"""
Lesson 12: graph.shortest_path(), the path itself, not just the distance.
...
def main() -> None:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        # No edge_types parameter here, shortest_path searches every
        # registered edge type and returns the first shortest match it
        # finds, see the README for what that means in practice.
        rows = conn.execute("""
    ...
    print("Shortest path, Dan -> Alice:")
    for step, node_table_name, node_id, edge_label in rows:
        suffix = f"  edge_label={edge_label}" if edge_label else ""
        print(f"  step={step}  {node_table_name:<10} {node_id}{suffix}")
