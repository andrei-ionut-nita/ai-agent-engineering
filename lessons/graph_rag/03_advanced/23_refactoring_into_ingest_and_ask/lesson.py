"""
Lesson 23: two functions, ingest() and ask(), matching this repo's
shared cross-course Strategy protocol (docs/RAG-SERIES-PLAN/README.md).

State = (graph, collection): a networkx.DiGraph of extracted triples,
and a chromadb.Collection indexing that graph's node names for
starting-entity lookup.
...
def ask(query: str, state: State, k: int = 2) -> str:
    # k is accepted for interface compatibility with this series' shared
    # Strategy protocol; this course's traversal depth is fixed by
    # Lesson 14's tuning, not driven by k the way naive_rag's ask() is.
...
def main() -> None:
    ...
    state = ingest(docs)
    print(f"Ingested a graph with {state.graph.number_of_nodes()} nodes.\n")
