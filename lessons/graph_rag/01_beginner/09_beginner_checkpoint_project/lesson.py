"""
Lesson 9: Beginner checkpoint, a CLI that builds a graph from a folder
of notes and answers connection questions across it. Nothing new here,
this combines Lessons 3-8's ideas into one script.
...
def main() -> None:
    print("Building graph from every fixture note...")
    graph = build_graph(NOTES_DIR)
    print(f"Graph has {len(graph)} nodes.\n")

    questions = [
        ("What tools are kept on the garage electronics bench?", "electronics bench"),
        (
    ...
    for query, start_entity in questions:
        print(f"Q: {query}")
        print(f"A: {ask(query, graph, start_entity)}\n")
